...
{
    my $line = shift;
    #reg expr for a inst line in sass.
    if ($line =~ m"^\s+/\*(?<num>[0-9a-f]+)\*/\s+$InstRe\s+/\* (?<code>0x[0-9a-f]+)"o)
    {
        return {
            #num is the start position(hex,in bytes) of inst in one text kernel section.
            num     => hex($+{num}),
            #pred is '@!Pxxx ' or '@Pxxx '
            pred    => $+{pred},
            #op is operation in string(before ' ')
            op      => $+{op},
            #rest contains all characters starting from the space following op to ';'
            ins     => normalizeSpacing($+{op} . $+{rest}),
            inst    => normalizeSpacing($+{pred} . $+{op} . $+{rest}),
            #code is the 64bit instruction value.
            code    => hex($+{code}),
        };
    }
    return undef;
}
#if line is a line of ctrl_code,parse ctrlcode for each inst.
sub processSassCtrlLine
{
    my ($line, $ctrl, $ruse) = @_;
    #m'' is similar to m//,the matching part in the first () will be stored in $1
    return 0 unless $line =~ m'^\s+\/\* (0x[0-9a-f]+)';

    my $code = hex($1);
    # ref EXPR return a non-empty string if EXPR is a ref,the empty string otherwise.
    if (ref $ctrl)
    {
        #get control code of every instruction.
